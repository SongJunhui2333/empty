...
/*                             模式2：第二问代码                            */
/* ---------------------------------------------------------------- */
uint32_t question2_start_time = 0;                                     // 记录模式2开始的时间
...
// uint16_t QUESTION2_MOTOR_BASE_SPEED = 70; // 模式2的电机基础速度
...
void Mode2_Loop(void)
{
    ...
    //         QUESTION2_HEADING_KP = cmd.value;
    //         pid_set_param(&question2_pid_heading, QUESTION2_HEADING_KP, QUESTION2_HEADING_KI, QUESTION2_HEADING_KD);
    //         sprintf((char *)uart_tx_buff, "OK KP=%.3f\r\n", (double)cmd.value);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_SET_KI:
    //         QUESTION2_HEADING_KI = cmd.value;
    //         pid_set_param(&question2_pid_heading, QUESTION2_HEADING_KP, QUESTION2_HEADING_KI, QUESTION2_HEADING_KD);
    ...
    //         QUESTION2_HEADING_KD = cmd.value;
    //         pid_set_param(&question2_pid_heading, QUESTION2_HEADING_KP, QUESTION2_HEADING_KI, QUESTION2_HEADING_KD);
    ...
    //         pid_set_setpoint(&question2_pid_heading, cmd.value);
    //         sprintf((char *)uart_tx_buff, "OK SET=%.1f\r\n", (double)cmd.value);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_SET_SPEED:
    //         QUESTION2_MOTOR_BASE_SPEED = (uint16_t)cmd.value;
    //         sprintf((char *)uart_tx_buff, "OK SPD=%d\r\n", QUESTION2_MOTOR_BASE_SPEED);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_RESET:
    //         pid_reset(&question2_pid_heading);
    //         pid_set_setpoint(&question2_pid_heading, QUESTION2_HEADING_SETPOINT);
    //         pid_set_param(&question2_pid_heading, QUESTION2_HEADING_KP, QUESTION2_HEADING_KI, QUESTION2_HEADING_KD);
    //         QUESTION2_MOTOR_BASE_SPEED = 60;
    //         UART_print_string(DEBUG_INST, "OK RESET\r\n");
    //         break;
    //
    //     case VOFA_CMD_PRINT_PARAMS:
    //         sprintf((char *)uart_tx_buff, "Q2: KP=%.3f KI=%.4f KD=%.1f SPD=%d\r\n", (double)question2_pid_heading.kp,
    //                 (double)question2_pid_heading.ki, (double)question2_pid_heading.kd, QUESTION2_MOTOR_BASE_SPEED);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     default:
...
#define QUESTION3_NUDGE_MAX 20 /* 微扰累积上限 */
...
void Mode3_Init(void)
{
    ...
    pid_init(&question3_pid_motor, PID_POSITION, QUESTION3_POS_KP, QUESTION3_POS_KI, QUESTION3_POS_KD,
...
void Mode3_Loop(void)
{
    ...
    if (q3_test_flag && uart_maixcam_rx_done)
    {
        ...
        /* 微扰：球静止且偏差过大时叠加推动 */
        if (abs(ball_pos - (int16_t)question3_pid_motor.setpoint) > 15 && abs(ball_vel) < QUESTION3_VEL_STILL)
        {
            nudge += (ball_pos > (int16_t)question3_pid_motor.setpoint) ? -QUESTION3_NUDGE_STEP : QUESTION3_NUDGE_STEP;
            if (nudge > QUESTION3_NUDGE_MAX)
                nudge = QUESTION3_NUDGE_MAX;
            if (nudge < -QUESTION3_NUDGE_MAX)
                nudge = -QUESTION3_NUDGE_MAX;
        }
        else
        ...
        /* -85 到位 + 低速 + 稳定计时 */
        else if (q3_state == Q3_GO_MINUS && q3_absf(pos_error) <= QUESTION3_POS_TOL &&
...
void Mode4_Init(void)
{
    ...
    pid_init(&question4_pid_motor, PID_POSITION, QUESTION4_MOTOR_KP, QUESTION4_MOTOR_KI, QUESTION4_MOTOR_KD,
             QUESTION4_MOTOR_OUTPUT_MAX, QUESTION4_MOTOR_OUTPUT_MIN);
    pid_set_setpoint(&question4_pid_motor, 0.0f); // 设置球杆系统的目标位置为0
...
static float QUESTION5_MOTOR_KP = 2.7f;   // 比例系数：球偏差 → 电机脉冲
static float QUESTION5_MOTOR_KI = 0.005f; // 积分系数：缓慢消除稳态误差（抗饱和已内置）
static float QUESTION5_MOTOR_KD = 40.0f;  // 微分系数：利用球速提供阻尼
// static float QUESTION5_PIPE_FF_GAIN = 0.2f;        // 水管弓形前馈增益 (脉冲/像素)
...
const float question5_trace_weights[8] = {-6, -4, -3, -1, 1, 3, 4, 6}; // 模式5的循迹权重数组
...
static float QUESTION5_HEADING_KI = 0.005f;           // 积分系数: 加速消除稳态偏置
static float QUESTION5_HEADING_KD = 25.0f;            // 微分系数: 增强阻尼防振荡
...
void Mode5_Loop(void)
{
    ...
    if (uart_maixcam_rx_done)
    {
        ...
        // /* ---- 水管弓形前馈补偿 ---- */
        // /* 水管跨度25cm, 中心下弯1mm, 前馈抵消重力分量, 减轻积分负担 */
        // motor_pos += QUESTION5_PIPE_FF_GAIN * (float)ball_error;

// ---- 微扰逻辑：小球静止且偏差 >15 时，逐步叠加微扰推动小球 ----
#define QUESTION5_VEL_STILL 20 // 判定小球静止的速度阈值
#define QUESTION5_NUDGE_STEP 5 // 每次微扰增量（脉冲）
#define QUESTION5_NUDGE_MAX 20 // 微扰累积上限
        ...
        // /* ---- VOFA+ 实时数据: Mode 5 控球PID (5通道 JustFloat) ---- */
        // /* ch0:目标值  ch1:小球偏差  ch2:PID输出  ch3:最终电机位置  ch4:小球速度 */
        // vofa_tx_busy = 1;
        // {
        //     float vofa_data[5];
        //     vofa_data[0] = question5_pid_motor.setpoint;
        //     vofa_data[1] = (float)ball_error;
        //     vofa_data[2] = question5_pid_motor.output;
    ...
    // /* ---- VOFA+ 调参命令处理 (Mode 5 转向PID + 控球PID) ---- */
    // if (vofa_cmd_available())
    // {
    //     vofa_cmd_t cmd = vofa_get_cmd();
    //     switch (cmd.type)
    ...
    //
    //     /* ---- 控球 PID 命令 ---- */
    //     case VOFA_CMD_SET_BALL_KP:
    //         QUESTION5_MOTOR_KP = cmd.value;
    //         pid_set_param(&question5_pid_motor, QUESTION5_MOTOR_KP, QUESTION5_MOTOR_KI, QUESTION5_MOTOR_KD);
    //         sprintf((char *)uart_tx_buff, "OK BALL_KP=%.3f\r\n", (double)cmd.value);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_SET_BALL_KI:
    //         QUESTION5_MOTOR_KI = cmd.value;
    //         pid_set_param(&question5_pid_motor, QUESTION5_MOTOR_KP, QUESTION5_MOTOR_KI, QUESTION5_MOTOR_KD);
    //         sprintf((char *)uart_tx_buff, "OK BALL_KI=%.4f\r\n", (double)cmd.value);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_SET_BALL_KD:
    //         QUESTION5_MOTOR_KD = cmd.value;
    //         pid_set_param(&question5_pid_motor, QUESTION5_MOTOR_KP, QUESTION5_MOTOR_KI, QUESTION5_MOTOR_KD);
    //         sprintf((char *)uart_tx_buff, "OK BALL_KD=%.1f\r\n", (double)cmd.value);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_SET_BALL_FF:
    //         QUESTION5_PIPE_FF_GAIN = cmd.value;
    //         sprintf((char *)uart_tx_buff, "OK BALL_FF=%.3f\r\n", (double)cmd.value);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     case VOFA_CMD_RESET_BALL:
    //         pid_reset(&question5_pid_motor);
    //         pid_set_setpoint(&question5_pid_motor, 0.0f);
    //         pid_set_param(&question5_pid_motor, QUESTION5_MOTOR_KP, QUESTION5_MOTOR_KI, QUESTION5_MOTOR_KD);
    //         UART_print_string(DEBUG_INST, "OK BALL_RESET\r\n");
    //         break;
    //
    //     case VOFA_CMD_PRINT_BALL_PARAMS:
    //         sprintf((char *)uart_tx_buff, "Q5 BALL: KP=%.3f KI=%.4f KD=%.1f SET=%.1f\r\n",
    //                 (double)question5_pid_motor.kp, (double)question5_pid_motor.ki, (double)question5_pid_motor.kd,
    //                 (double)question5_pid_motor.setpoint);
    //         UART_print_string(DEBUG_INST, (char *)uart_tx_buff);
    //         break;
    //
    //     default:
...
const float question6_trace_weights[8] = {-6, -4, -3, -1, 1, 3, 4, 6}; // 模式6的循迹权重数组
uint16_t question6_current_speed = 0;                                  // 模式6当前的电机速度目标值

pid_t question6_pid_heading;                          // 模式6的PID控制器实例，用于调整小车的转向
static float QUESTION6_HEADING_KP = 2.7f;             // 比例系数: 提高弯道响应
static float QUESTION6_HEADING_KI = 0.005f;           // 积分系数: 加速消除稳态偏置
static float QUESTION6_HEADING_KD = 40.0f;            // 微分系数: 增强阻尼防振荡
...
#define QUESTION6_NUDGE_MAX 20 // 微扰累积上限
...
void Mode7_Loop(void)
{
    ...
    if (KeyNum == 3) // 按键3被按下
    {
        ...
        {
            NextMode = 9; // 切换到模式9，调节第三问目标位置
        }
        else if (Param_Select_Num == 3)
        {
...
static uint8_t q9_edit_select = 0; /* 0=编辑 TARGET_PLUS, 1=编辑 TARGET_MINUS */

void Mode9_Init(void)
{
    OLED_Clear();
    OLED_ShowString(0, 0, (uint8_t *)"Adj Q3 Target", 16);
...
void Mode9_Loop(void)
{
    ...
    {
        if (q9_edit_select == 0)
            QUESTION3_TARGET_PLUS += 5.0f;
        else
            QUESTION3_TARGET_MINUS += 5.0f;
    }
    /* ---- Key 2: 当前选中值 -5 ---- */
    else if (KeyNum == 2)
    {
    ...
    OLED_ShowString(0, 6, (uint8_t *)"K3:Switch", 8);

    if (q9_edit_select == 0)
        OLED_ShowString(0, 7, (uint8_t *)">>PLUS<<", 8);
    else
        OLED_ShowString(0, 7, (uint8_t *)">>MINUS<<", 8);
}
...
void Mode10_Loop(void)
{
    ...
    {
        /* ---- 阈值调节模式 ---- */
        if (KeyNum == 1 && QUESTION2_BLACK_LINE_THRESHOLD < 8)
        {
            QUESTION2_BLACK_LINE_THRESHOLD++;
        }
        else if (KeyNum == 2 && QUESTION2_BLACK_LINE_THRESHOLD > 1)
        {
            QUESTION2_BLACK_LINE_THRESHOLD--;
        }
        else if (KeyNum == 4)
